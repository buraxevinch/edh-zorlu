...
const Treatment = ({ data, dict }) => {
  const { cats, home, image, lang, left, text, video } = data;
  const { cat, desc, short, subttl, ttl } = text;
  const root = process.env.NEXT_PUBLIC_MEDIA_URL + "/public/images/";

  const handleLeft = () => {
    const options = cats.map((c, i) => ({ label: c, value: i.toString() }));
    ...
    const formSchema = {
      ...
      fields: [
        ...
        { type: "select", name: "cats", options, rules: [{ type: "required" }] },
          ...
          <div>
            {left[0].title && (
              <div className="mb-4 border-b before:-bottom-[0.05rem]">
                <h4 className="text-lg font-normal">{left[0].title}</h4>
              </div>
            )}
        ...
        <div className="container">
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex items-center gap-2 text-xs">
              <Link href="/" className="btn-mine text-sky-600">
                {home}
              </Link>
        ...
        <div className="flex flex-grow basis-4 2xl:gap-10 gap-8">
          ...
          <div className={`basis-full${left ? " xl:basis-3/4" : ""}`}>
            <div className="grid grid-cols-1 2xl:gap-8 gap-6 overflow-hidden">
              {short && <h3 className="2xl:text-2xl text-xl">{short}</h3>}
              {image && left && <Image src={root + image.src} alt={ttl} width={image.wdt} height={image.hgh} placeholder="blur" blurDataURL={image.base} className="w-full rounded-2xl" />}
              {video && <OpenVideo title={ttl} video={video} />}
